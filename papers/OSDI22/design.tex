...
% atomic operations provided a serialized order on writes, and prevent torn
% reads on the atomic region.
...
%%
This value is application specific; in Clovers case 64 zero bits indicate
...
\subsection{Instruction Coalescing} RoCE coalesces some ACKs as an optimization.
Multiple writes can be acknowledged by a single acknowledgement, in the form of
either an ACK, ATOMIC ACK, or Read Response. This occurs when multiple RDMA
request are processed concurrently. When mapping requests, some acknowledgements
may be coalesced which are required from the senders perspetive.
