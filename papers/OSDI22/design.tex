...
dramatically decrease the likilihood of a failed operation due to a
...
processed by teh server as usual, any subsequent reordering will
simply cause the operation to fail, just as it would have without the
reordering.
...
If the serializer is able to ensure that packets will not be reordered
between when it operates on them and their arrival at the memory
server--because, for example, it is directly connected over a single
link--we can leverage the ordering semantics provided by RDMA queue
pairs to further reduce---and even eliminate---failed and/or
