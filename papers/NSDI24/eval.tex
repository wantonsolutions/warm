...
succeeds in making an update. On our testbed FUSEE scaled to
...
Figure~\ref{fig:full_system_performance} shows the impact of
\sword's techniques at various levels of contention.  A
read-only workload exhibits no contention, so \sword\ simply
passes through all operations unmodified achieving a maximum
throughput of approximately 40 million operations per second
in our testbed.  Clover (shown in blue) performance
decreases markedly with even 5\% writes; write steering
...
workload write steering alone is sufficient. FUSEE (green)
suffers less from contention as it does not maintain a
linearized log. However due to it's additional meta-data
updates reads take two round trips, while writes take three
or more. \sword achieves between 1.9x and 2.5x higher
