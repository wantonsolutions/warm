...
To improve performance, Clover moves metadata storage off of the data
path. On the data path, reads and writes for a given key are made to
an append-only linked list stored in remote memory. All RDMA requests
...
fails to land at the tail of the list Clover iteratively traverses the
...
keep caches (i.e., hints) of the end of each key's linked list to
avoid traversals.
