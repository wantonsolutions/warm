...
A max of 4k connections are supported. The buffer size
determines the maximum number of in-flight requests per QP
(128 in \sword).  Prior work has demonstrated that RDMA
ICRC's can be implemented in a P4 switch, but are redundant
due to ethernet CRC ~\cite{p4telemetry,bedrock}.
Hence, like
previous authors~\cite{switchml}, we disable ICRC checks on
both the sender and receiver NICs to save switch resources in our experiments.
...
the Barefoot SDE version 9.7.0. Each reported percentage is
...
We use the header parser from the P4 simple switch to parse
up to the UDP header and create our own header parser for
RoCEv2 and Clover headers.  {\sword} uses RoCEv2 header, and
write and CAS payload information to identify traffic for
steering and multiplexing. When Clover traffic is identified
the connection is added to a connection tracker and a
...
\begin{figure}
  \center
    ...
    \caption{\sword's DPDK packet-processing pipeline.}
    \label{fig:system}
  \vskip -0.5em
\end{figure}
...
\textbf{Connection mapping.} Map entries are required for
each packet in order to multiplex a response packet back
onto its original connection.  In the worst case (an
all-client incast) a connection may have a mapped entry for
every connected client. Because switch registers are
allocated at compile time, statically provisioning for the
worst case requires $O(n^2)$ space complexity ($n$ entries
for every connection).  Supporting only 512 clients requires
2MB of SRAM (1/16th of a Tofino's total buffer space) given
this constraint. Storing these 64-bit entries takes 2
pipeline stages in the best case, and 8 in the worst (8-bit
registers).
...
\textbf{Request coalescing.} As an optimization Rdma NICs
...
Implementing this functionality in P4 is extremely
...
%%
At the time of writing turning off coalescing is not
possible on Mellanox NICs. Turning off ack coalescing would
dramatically reduce the complexity required to multiplex
connections.
% Of
