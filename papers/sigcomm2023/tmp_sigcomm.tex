...
\vskip -0.5em
    ...
    \caption{ RC multiplexing in \sword. Per connection in
    and out sequence numbers are tracked to decouple sending
    and receiving QP. Map entries are stored per packet in
    their destination connections ring buffer to map back
    responses. The diagramed rings only show the original
    connection id, and sequence number. This example shows 3
    packets mapped to connection 1, one from each
    connection. A single packet from connection 2 remains
    unmapped but still tracked.  }
