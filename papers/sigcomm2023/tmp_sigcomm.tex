...
4k connections are supported.  Each connection stores IP addresses,
ports, Ethernet MAC addresses, and RDMA sequence numbers. All
packets generate map entries when passing through a tracked
connection. The map entry stores the sequence number of the
packet, and the packet's original QP id. When a packet is
...
Figure~\ref{fig:qp_mapping}. Note that connections are not terminated
at the switch, only sequence numbers are tracked dynamically. Complex
tasks such as retransmission, setup and teardown are handled end-to-end by the RDMA NICs.
%%
