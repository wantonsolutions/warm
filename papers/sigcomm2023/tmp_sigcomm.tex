...
RoCE packets have two headers, BTH,and BTH+, shown in
Figure~\ref{fig:roce_header}, both of which \sword\ needs to parse.
The BTH header contains determines the operation, while the BTH+
headers contain virtual addresses an operation executs on, the
operation itself, and its payload.
...
A max of 4k connections are supported.

In cases where \sword\ wishes to enforce ordering across operations
from different clients, it can multiplex them onto a single, shared
reliable connection it establishes with the server.

\paragraph{Connection tracking}
%
\sword\ tracks all reliable conneciton queue pairs and maintains a
table of connection metadata shown in Figure~\ref{fig:qp_mapping}.
Entries are added upon connection establishment and removed on
teardown dynamically. Each entry is uniquely identified by its source
and destination Ethernet MACs, IP addresses, and UDP ports in
combination with its queue pair ID.  As packets associated with the queue pair flow through the switch, the mapping table is updated with the 


, Ethernet MAC addresses, and
...
%tail latencies.
%%
\sword\ detect coalescing by monitoring response
