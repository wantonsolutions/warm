...
De-multiplexing mapped requests non-trivial due to
optimizations in the RDMA protocol. Acks in RDMA, can be
coalesced to reduce packets, and bandwidth. Coalescing is a
problem when multiplexing to two or more clients. A request
from one client, may cause the coalescing of a result for
another. If this occurs the second client will not receive a
response and eventually time out, leading to retransmission
and high tail latencies.
%%
We detect coalescing on the switch by monitoring response
