...
point, even modest levels of write contention crater the performacne
...
protocol provides two basic mechanisms to do so remotely: reliable
connections that ensure ordering and atomic operations that deliver
mutual exclusion.  While the scalability of RDMA connection
handling---once a point of significant criticism~\cite{}---has
improved markedly in modern hardware, connections remain an end-to-end
abstraction, and do not provide any guarantees regarding operations
from distinct clients.  For that, systems must rely on atomic
operations like compare-and-swap (CAS), but their enhanced semantics
dictate expensive implementation choices on the NIC, dramatically
restricting their performance compared to simple verbs like read and
write~\cite{design-guidelines}.  Moreover, atomic operations are 
...
operations between multiple clients~\cite{fasst}, avoiding the need
...
abstraction and providing more powerful verbs~\cite{rma}---but such
hardware is not commercially available.
...
exlusively on end-to-end solutions, we consider leveraging in-network
resources---specifially programmable switches that are located between
...
RDMA updates to avoid write/write conflicts in append-only data structures.

We apply \sword\ to two existing remote memory systems which natively
support sharing: Sherman~\cite{sherman}, and Clover~\cite{clover}.
Sherman employs locks, while Clover relies on optimistic concurrency.
We show that both systems collapse under contention due to RDMA's
limitations, but \sword\ removes the existing bottlenecks.  Concretely, 
...
index structure by steering requests to a serialized set of locations,
