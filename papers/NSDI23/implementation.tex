...
supports conflict avoidance (\S\ref{ca}) in Clover at 100-Gbps line
rate, but does not perform the queue-pair remapping necessary to
enforce ordering (\S\ref{ea}) that our full-featured DPDK
...
Steering is straightforward to implement in P4. We use P4 \textit{registers} to
store connection state, virtual addresses, and outstanding requests. Switch
registers are constrained to 32, 16 and 8 bit blocks, and are bound to specific
switch pipeline stages~\cite{netcache}. Packets visit each stage exactly once,
so register reads and writes must be pipelined correctly so that the same stage
which stores a virtual address on a write, is the same that produces the address
for CAS and read. Because registers are fixed width, some lookups take multiple
stages.  We use two stages lookups for virtual addresses (64 bits) with two 32
bit registers, and a single stage for queue pairs, sequence numbers, and
connection id's. {\sword} switch resource utilization is detailed in
...
limitations on the programmable switch, and on ConnextX-5 NICs. Implementing
{\sword} in DPDK is simple as it is written in C. In this section we describe
why tracking outstanding requests and un-coalescing ACKs is made untenable by
current hardware, and suggest small hardware modifications which would make the
implementation simple.

\textbf{Connection Mapping:} When a packet is mapped to a queue pair a 64 bit
map entry is required to map the response back to it's sender. In the worst case
a connection may have a mapped entry for every connected client. Because switch
registers are allocated at compile time, statically provisioning for the worst
case requires $O(n^2)$ space complexity ($n$ entries for every connection).
Supporting only 512 clients requires 2MB of SRAM (1/16 a Tofino's total buffer
space) given this constraint. Storing these 64 bit entries takes 2 pipeline
stages in the best case, and 8 in the worst case (8bit registers).

In the worst number of in flight requests with $n$ close looped clients is $n$,
so $n^2$ is dramatic over provisioning. However, space efficient alternatives
are even harder to implement. A hash table for instance has much better space
utilization, but collisions are impractically hard to deal with. For each
potentially collision, additional pipeline stages must be allocated, and entries
must include QP ids to check for collisions. This increases a single hash table
lookup to 4 stages, which is multiplied for each potential collision. At
millions of packets per second collisions of 3 or more are common which exceeds
the pipeline length of Tofinio switches.

\textbf{(Fix) Dynamic Memory Allocation:} 
%%
If registers could be resized at runtime, or dynamically mapped with a virtual
we could resize our entry tables at runtime and steal from space from under
utilized connections. This has the potential to decrease our memory utilization
to clost to $O(n)$. This is a trivial task with an allocator and pointers, but a
prohibitively hard task with static registers and single stage lookups.
...
\textbf{Request Coalescing:} is the largest barrier to implementing connection
mapping in P4. Recall that memory side NICs frequently summarize multiple ACKs
into a single ACK. When mapping, these ACKs must be iteratively reconstructed if
the original requests were generated by different clients.
%%
In P4 this functionality is prohibitively hard to engineer. A nondeterministic
number of entries must be looked up for every packet (up to the number of
clients). In P4 each stage of the pipeline holds unique data and supports a
single lookup of the specified register size (8,16,32), as such we are unable to
look up a variable number of requests to generated coalesced ACKs.

Duplicating mapped entries into multiple stages would allow for multiple lookups
per packet. However as mentioned above map entires consume 2MB of SRAM. Further,
an arbitrary number of ACKs can be coalesced so no fixed number of duplications
would be sufficient (Acks coalescing 10 or more requests occured frequently). A
final alternative is to recirculate packets to attempt to trigger prior acks,
however this unnecessarily inflates bandwidth in the common case, and would
cause responses to be delivered in reverse order.

\textbf{(Fix) Turn off Coalescing:} None of the coalescing complexity is
required if the performance optimization could be turned of as a NIC feature. As
coalescing is not strictly required we hope future RDMA NICs will make it a
