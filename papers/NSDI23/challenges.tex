...
exclusion across all RDMA queue pairs (RDMA connections). Concurrent read and
write operations with a data dependency on the atomic address must stall until
the atomic completes. Atomic operations issued to host memory require two PCIe
round trips to execute before dependent operations can be unblocked.
%%
...
in order execution.  Wether executed on host memory or nic mapped, writes on a
single queue pair significantly out perform locking operations.
...
\sg{This can be trimmed down now}
