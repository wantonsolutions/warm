  ...
  \centering
   \vskip -0.5em
    \caption{Average number of bytes required per 128-byte Clover
      operation using each of the three techniques at various write
      intensities.}
...
%\vskip -1em
    \caption{99th-percentile tail latencies of read (solid) and write
      (striped) Clover operations at various write
      intensities.  Measurements taken over a Zipf distribution of requests with 64
      clients. (Note logarithmic $y$ axis.)}
...
\subsection{Partial key tracking}

{\sword} does not need to track all of a programs state. If a programs state is
too large to cache on a switch, but a few hot keys are highly contested,
{\sword} can provide significant benefits. In the following experiment we track
a subset of Clovers keys, allowing the others to flow uninterrupted through
\sword. In this experiment we run clover at a Zipf of 1.5 (most requests are on
lower keys). We add keys in order of hotness in groups of 8. At this
distribution tracking only the hottest 8 keys provides a 9.5x performance
improvement, and tracking 64 provides 27x.

\begin{figure}
    \includegraphics[width=0.485\textwidth]{fig/keys_tracked.pdf}
%\vskip -1em

    \caption{ Performance improvement from tracking subsets of clovers keys.
    Note that at 0 keys, the experiment is identical to running default Clover.
    Performance is averaged across 6 hosts (56 threads each) at zipf of 1.5.  }
