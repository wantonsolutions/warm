...
%  \vskip -0.5em
    \caption{Steering applied to Clover with 128-byte packets on 4 YCSB
...
In the default case (labeled CAS), {\sword} lets CAS requests flow
...
\begin{figure}
  ...
  \centering
   \vskip -0.5em
    \caption{Average number of bytes required per 128-byte Clover
      operation using each of the three techniques for various write
      intensities.}
...
%\vskip -1em
    \caption{99th-percentile tail latencies of read (solid) and write
      (striped) Clover operations for workloads of various write
      intensities.  Each
      measurement taken over a Zipf distribution of requests with 64
      clients. (Note the logarithmic $y$ axis.)}
...
dividing by the total number of operations. Clover's bandwidth usage
increases with contention, growing by 1.8$\times$ at 50\% and
2.24$\times$ at 100\% writes---all of which is recovered by applying
...
(solid blue) at 5\% writes is 91~$\mu$s, around 7.5$\times$ its
baseline our our testbed. With read and write steering (solid red) the read tail
latency drops to 38~$\mu$s---a 2.3$\times$ improvement over Clover
even in this low-contention regime. At 50\% writes the performance
increase from steering roughly doubles again.
...
%explicitly known.
%
Writes (hashed) perform worse than reads (solid) under contention in
default Clover. Combined write and read steering provides a 2.7, 37.1,
and 25.2$\times$ improvement in write tail latency, respectively, across 5,
50, and 100\% write workloads.  As one might expect, performing write
steering alone privledges writes over reads, dropping their tail
latencies slightly further---at the cost of a dramatic spike in read
tail latency.
