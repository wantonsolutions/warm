...
#define APP_LCORE_WORKER_FLUSH       1
#endif
    ...
    cuckoo_hashtable_t * table)
{
    ...
    //static struct rte_mbuf last_pkt;
	...
	for (i = 0; i < lp->n_rings_in; i ++) {
        ...
        /*
		if (unlikely(ret == 0) && last_pkt_known == 0) {
			continue;
        } else if (unlikely(ret ==0) && last_pkt_known == 1) {
            //printf("About to inject a pkt to send\n");
            if ( lp->mbuf_in.array[0] != NULL ) {
                lp->mbuf_in.array[0] = &last_pkt;
            } else {
                printf("Unable to send packet, buffer is NULL\n");
            }
        }*/
...
app_lcore_main_loop_worker(void) {
    ...
    tracker.second_per_sample = 4;


    cuckoo_hashtable_t* table = cuckoo_init(19); //TODO make this fit to some fixed size for keys and values
	...
	for ( ; ; ) {
		...
		app_lcore_worker(lp, bsz_rd, bsz_wr, &tracker, table);
