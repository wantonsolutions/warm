...
static inline bool is_slot_empty(cuckoo_hashtable_t* h,
                                 size_t i,
                                 size_t j) {
    if (eq_int_key(TABLE_KEY(h, i, j),0))
        return true;
    return false;
}
...
static int _cuckoopath_search(cuckoo_hashtable_t* h,
    ...
    while ((h->kick_count < MAX_CUCKOO_COUNT) &&
        ...
        for (idx = 0; idx < NUM_CUCKOO_PATH; idx ++) {
            ...
            j = rand() % bucketsize;
            curr->slots[idx] = j;
            copy_key((void *)curr->keys[idx],(void *)TABLE_KEY(h, i, j));
...
static int _cuckoopath_move(cuckoo_hashtable_t* h,
    ...
    while (depth > 0) {
        ...
        copy_key((void *)TABLE_KEY(h, i2, j2),(void *)TABLE_KEY(h, i1, j1));
        copy_value((void *)TABLE_VAL(h, i2, j2),(void *)TABLE_VAL(h, i1, j1));
        copy_key((void *)TABLE_KEY(h, i1, j1),(void *)empty_key);
        copy_value((void *)TABLE_VAL(h, i1, j1),(void *)empty_value);
...
static bool _try_del_from_bucket(cuckoo_hashtable_t* h,
    ...
    for (j = 0; j < bucketsize; j ++) {
        ...
        if (keycmp((char*) TABLE_KEY(h, i, j), key)) {
            ...
            copy_key((void*)TABLE_KEY(h, i, j),(void *) empty_key);
            copy_value((void **)&TABLE_VAL(h, i, j),(void **)&empty_value);
            /* buckets[i].keys[j] = 0; */
            /* buckets[i].vals[j] = 0; */
...
void copy_key(void * a, void * b) {
    memcpy(a,b,sizeof(KeyType));
}

void copy_value(void *a, void *b) {
    //printf("start %s\n",__func__);
    memcpy(a,b,sizeof(ValType));
    //printf("end %s\n",__func__);
}
